  ...
  return (
    <div className={"group relative " + (className ?? "")}>
      ...
      <div className="overflow-hidden">
        <ul
          className="flex items-center gap-6 py-4 animate-marquee [animation-play-state:running] group-hover:[animation-play-state:paused]"
          style={{
        ...
        >
          {duplicated.map((logo, index) => (
            <li key={`${logo.src}-${index}`} className="flex-none px-10">
              <Image
                src={logo.src}
                alt={logo.alt}
                width={240}
